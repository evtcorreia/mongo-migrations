...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your Mongo Migrations account.<br>
        ...
        &mdash; Mongo Migrations Team
      `
      return sendMail({ toEmail: email, subject: 'Mongo Migrations - Password Reset', content })
    })
